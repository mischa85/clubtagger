...
CFLAGS   ?= $(CSTD) $(OPT) $(WARN) $(THREAD) $(ALSA_CFLAGS) $(CURL_CFLAGS)
LDFLAGS  ?= $(THREAD) $(ALSA_LIBS) $(CURL_LIBS) $(MATH_LIBS) $(VIBRA_LIBS)
...
debug: CFLAGS := -std=c11 -g -O0 -fno-omit-frame-pointer -fsanitize=address,undefined $(WARN) $(THREAD) $(ALSA_CFLAGS) $(CURL_CFLAGS) $(CFLAGS_EXTRA)
debug: LDFLAGS := $(THREAD) $(ALSA_LIBS) $(CURL_LIBS) $(MATH_LIBS) $(VIBRA_LIBS) -fsanitize=address,undefined $(LDFLAGS_EXTRA)
debug: clean $(APP)
